...
function [p,l,r] = solvechannelransac(A,B,C,weights,iters,mpe)
    ...
    nlights = size(A,2);
    data = [weights B C A];
    [params,r] = ransac(@ransacComputeParams, data, iters, (4+nlightparams*nlights), @(p,d) ransacInlierFunction(p,d,mpe));
    p = params(1);
    l = params(2:end);
    r = 1 - (r/size(data,1));
end
...
filename = 'eq.m';
run(filename);
[p0,l0,r0] = solvechannelransac(A,B0,C0,weights,900,0.05);
[p1,l1,r1] = solvechannelransac(A,B1,C1,weights,900,0.05);
[p2,l2,r2] = solvechannelransac(A,B2,C2,weights,900,0.05);
