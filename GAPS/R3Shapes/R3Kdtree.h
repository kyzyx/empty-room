...
public:
  ...
  PtrType FindClosest(PtrType point, RNLength min_distance = 0, RNLength max_distance = FLT_MAX, 
    RNLength *closest_distance = NULL) const;
  PtrType FindClosest(const R3Point& position, RNLength min_distance = 0, RNLength max_distance = FLT_MAX, 
    RNLength *closest_distance = NULL) const;

  // Search for closest K 
  int FindClosest(PtrType point, RNLength min_distance, RNLength max_distance, int max_points, 
    RNArray<PtrType>& points, RNLength *distances = NULL) const;
  int FindClosest(const R3Point& position, RNLength min_distance, RNLength max_distance, int max_points, 
    RNArray<PtrType>& points, RNLength *distances = NULL) const;

  // Search for all within some distance to a point
  int FindAll(PtrType point, RNLength min_distance, RNLength max_distance, 
    RNArray<PtrType>& points) const;
  int FindAll(const R3Point& position, RNLength min_distance, RNLength max_distance, 
    RNArray<PtrType>& points) const;
