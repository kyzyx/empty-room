...
FindClosest(R3KdtreeNode<PtrType> *node, const R3Box& node_box, const R3Point& position, 
  ...
  PtrType& closest_point, RNScalar& closest_distance_squared) const
{
  ...
  if (node->children[0]) {
    ...
    if (RNIsGreater(position.Z(), node_box.ZMax())) dz = position.Z() - node_box.ZMax();
    else if (RNIsLess(position.Z(), node_box.ZMin())) dz = node_box.ZMin()- position.Z();
    else dz = 0.0;
    ...
    for (int i = 0; i < node->npoints; i++) {
      ...
      RNLength distance_squared = R3SquaredDistance(position, Position(point));
...
FindClosest(const R3Point& position, 
...
template <class PtrType>
int R3Kdtree<PtrType>::
FindClosest(const R3Point& position, RNScalar min_distance, RNScalar max_distance, int max_points, 
  RNArray<PtrType>& points, RNLength *distances) const
{
  ...
  // Search nodes recursively
  FindClosest(root, bbox, position, min_distance_squared, max_distance_squared, max_points, points, distances_squared);
...
template <class PtrType>
int R3Kdtree<PtrType>::
FindClosest(PtrType point, RNScalar min_distance, RNScalar max_distance, int max_points, RNArray<PtrType>& points, RNLength *distances) const
{
  // Find closest within some distance
  return FindClosest(Position(point), min_distance, max_distance, max_points, points, distances);
}
...
template <class PtrType>
void R3Kdtree<PtrType>::
FindAll(R3KdtreeNode<PtrType> *node, const R3Box& node_box, const R3Point& position, 
...
template <class PtrType>
int R3Kdtree<PtrType>::
FindAll(const R3Point& position, RNScalar min_distance, RNScalar max_distance, RNArray<PtrType>& points) const
{
  // Check root
  if (!root) return 0;
