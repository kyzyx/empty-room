...
Eigen::Vector3f WallFinder::getWallEndpoint(int i, bool lo, double height) const {
    return fph.getWallEndpoint(i, lo, height);
}

Eigen::Vector3f WallFinder::getNormalizedWallEndpoint(int i, bool lo, double height) const {
    return fph.getNormalizedWallEndpoint(i, lo, height);
}

void WallFinder::getAsRoomModel(roommodel::RoomModel* rm) {
    fph.floorplane = floorplane;
    fph.ceilplane = ceilplane;
    fph.wallsegments = wallsegments;
    fph.forwards = forwards;
    fph.getAsRoomModel(rm);
}

void WallFinder::loadFromRoomModel(roommodel::RoomModel* rm) {
    wallsegments.clear();
    fph.loadFromRoomModel(rm);
    floorplane = fph.floorplane;
    ceilplane = fph.ceilplane;
    wallsegments = fph.wallsegments;
    forwards = fph.forwards;
}
