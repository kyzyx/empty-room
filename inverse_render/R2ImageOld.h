...
class Filter {
    public:
        Filter(void) : xdim(0), ydim(0), v(NULL) {}
        ...
        Filter(int d) {
            ...
            xdim = d;
            v = new double*[ydim];
            for (int i = 0; i < ydim; ++i) v[i] = new double[xdim];
        }
        Filter(int xd, int yd) {
            xdim = xd;
            ydim = yd;
            v = new double*[ydim];
            for (int i = 0; i < ydim; ++i) v[i] = new double[xdim];
        ...
        void reset(int d) {
            for (int i = 0; i < ydim; ++i) delete [] v[i];
            delete [] v;
            xdim = d;
            ydim = d;
            v = new double*[ydim];
...
class R2Image {
 public:
  ...
  R2Pixel applyFilterToPixel(int x, int y, Filter& k);
  void LinearFilter(Filter& k);
  static Filter& getGaussian(Filter& f, double sigma);
  void Blur(double sigma);
  void Sharpen(void);
  void EdgeDetect(void);
  void SobelEdgeDetect(void);
  ...
  R2Pixel Sample(double x, double y, int method, int extent, int boundary, bool filtered);
  R2Pixel SampleGaussian(double x, double y, int extent, bool filtered=false);
  R2Pixel SampleBilinear(double x, double y, bool filtered=false);
  R2Pixel SamplePoint(double x, double y, bool filtered=false);
  ...
  void Interpolate(const R2Image& image, double factor);
  void Interpolate(const R2Pixel& pixel, double factor);
  void Composite(const R2Image& top, int operation);
  void Dab(double d);
  void Mosaic(double d);
 ...
 private:
  ...
  static const double IMGGAMMA;
  R2Pixel& getKernelPixel(int i, int j, int x, int y, Filter& k);
  R2Pixel MedGetPixel(int x, int y);
};
