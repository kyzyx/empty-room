...
namespace Simplify
{
	...
	struct Triangle { int v[3];double err[4];int deleted,dirty;R3Vector n; };
	struct Vertex { R3Vector p;int tstart,tcount;SymmetricMatrix q;int border;};
	...
	double calculate_error(int id_v1, int id_v2, R3Vector &p_result);
	bool flipped(R3Vector p,int i0,int i1,Vertex &v0,Vertex &v1,std::vector<int> &deleted);
	...
	//
	void simplify_mesh(int target_count, double aggressiveness=7)
	{
		for (int i = 0; i < triangles.size(); ++i) triangles[i].deleted=0;
        ...
        for (int iteration = 0; iteration < 200; ++iteration)
		{
			...
			// clear dirty flag
			for (int i = 0; i < triangles.size(); ++i) triangles[i].dirty=0;
			...
			//
			double threshold = 1e-6*pow(double(iteration+3),aggressiveness);
	...
	bool flipped(R3Vector p,int i0,int i1,Vertex &v0,Vertex &v1,std::vector<int> &deleted)
	{
        ...
        for (int k = 0; k < v0.tcount; ++k)
		{
			...
			R3Vector d1 = vertices[id1].p-p; d1.Normalize();
			R3Vector d2 = vertices[id2].p-p; d2.Normalize();
	...
	void update_mesh(int iteration)
	{
		...
		if( iteration == 0 )
		{
			...
			{
				// Calc Edge Error
				Triangle &t=triangles[i];R3Vector p;
	...
	double calculate_error(int id_v1, int id_v2, R3Vector &p_result)
	{
		...
		else
		{
			...
			R3Vector p1=vertices[id_v1].p;
			R3Vector p2=vertices[id_v2].p;
			R3Vector p3=(p1+p2)/2;
